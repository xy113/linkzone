...
class Smarty_Compiler extends Smarty {
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
		...
		//==========================================================
		//===========================================================
		$compiled_content = str_replace('<!--<?php','<?php',$compiled_content);
		$compiled_content = str_replace('?>-->','?>',$compiled_content);
		$compiled_content = str_replace('<!-- <?php','<?php',$compiled_content);
		$compiled_content = str_replace('?> -->','?>',$compiled_content);
		
    	$tmp_dir = 'templates/'.$GLOBALS['cfg']['template'].'/';
		!defined('IN_ADMIN') && define('IN_ADMIN',false);
		if (!IN_ADMIN){
			/* 修正css路径 */
	        $compiled_content = preg_replace('/(<link\shref=["|\'])(?:\.\/|\.\.\/)?(images\/)?([a-z0-9A-Z_]+\.css["|\']\srel=["|\']stylesheet["|\']\stype=["|\']text\/css["|\'])/i','\1' . $tmp_dir . '\2\3', $compiled_content);
			$pattern = array(
	            '/<!--[^>|\n]*?({.+?})[^<|{|\n]*?-->/', // 替换smarty注释
	            '/<!--[^<|>|{|\n]*?-->/',               // 替换不换行的html注释
	            '/(href=["|\'])\.\.\/(.*?)(["|\'])/i',  // 替换相对链接
