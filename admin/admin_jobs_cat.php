 ...
 * ---------------------------------------------
 * $Date: 2010-12-18
 * $ID: admin_jobs_cat.php
*/ 
define("CURSCRIPT",'admin_jobs_cat');
...
if($inajax && $_GET['action']=='edit_name'){
    ...
    $db->query("UPDATE sdw_jobs_cat SET name='$name' WHERE cid=$cid");
    dexit($name);
}
...
if($inajax && $_GET['action']=='edit_kw'){
    ...
    $db->query("UPDATE sdw_jobs_cat SET keywords='$keywords' WHERE cid=$cid");
    dexit($keywords);
}
...
if($inajax && $_GET['action']=='edit_order'){
    ...
    $db->query("UPDATE sdw_jobs_cat SET displayorder='$displayorder' WHERE cid=$cid");
    dexit($displayorder);
}
...
if($_GET['action']=='save'){
    ...
    if ($cat['cid']>0){
	    $update_sql = "UPDATE sdw_jobs_cat SET cup='$cat[cup]',name='$cat[name]',
	    ...
	    $insert_sql = "INSERT INTO sdw_jobs_cat(cup,name,keywords,description,displayorder,directory,domain,disabled)VALUES".
...
if($_GET['action']=='drop'){
    ...
    if ($cid>0){
	    $catequery = $db->query("SELECT cid,name FROM sdw_jobs_cat WHERE cup='$cid' OR cid='$cid'");
	    while($categoryrs = $db->fetch_array($catequery)){
	    	$db->query("DELETE FROM sdw_jobs_cat WHERE cid=".$categoryrs['cid']);
	    	if ($islog){
...
if($_GET['action']=='edit'){
    ...
    $category = $db->get_one("SELECT * FROM sdw_jobs_cat WHERE cid=$cid");
    if(!empty($category)){
	...
	$smarty->assign('categories',get_jobs_category($cup));
}
$smarty->assign('manage_title',$LANG['jobs_cat_manage']);
...
function checkcategory($name){
	global $db;
	if (!empty($name)){	
	    $category = $db->get_one("SELECT name FROM sdw_jobs_cat WHERE name='$name'");
