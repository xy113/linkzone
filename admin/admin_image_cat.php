 ...
 * ---------------------------------------------
 * $Date: 2010-08-15
 * $ID: admin_image_cat.php
*/ 
define("CURSCRIPT",'admin_image_cat');
...
if($inajax && $_GET['action']=='edit_name'){
    ...
    $db->query("UPDATE sdw_image_cat SET name='$name' WHERE cid=$cid");
    dexit($name);
}
...
if($inajax && $_GET['action']=='edit_kw'){
    ...
    $db->query("UPDATE sdw_image_cat SET keywords='$keywords' WHERE cid=$cid");
    dexit($keywords);
}
...
if($inajax && $_GET['action']=='edit_order'){
    ...
    $db->query("UPDATE sdw_image_cat SET displayorder='$displayorder' WHERE cid=$cid");
    dexit($displayorder);
}
...
if ($inajax && $_GET['action']=='toggle_disabled'){
	...
	$db->query("UPDATE sdw_image_cat SET disabled='$disabled'  WHERE cid=$cid");
	dexit($disabled);
}
//================================
/**保存分类信息**/
...
if($_GET['action']=='save'){
    ...
    if ($cat['cid']>0){
	    $update_sql = "UPDATE sdw_image_cat SET cup='$cat[cup]',name='$cat[name]',
	    ...
	    $insert_sql = "INSERT INTO sdw_image_cat(cup,name,keywords,description,displayorder,directory,domain,disabled)VALUES".
...
if($_GET['action']=='drop'){
    ...
    if ($cid>0){
	    $catequery = $db->query("SELECT cid,name FROM sdw_image_cat WHERE cup='$cid' OR cid='$cid'");
	    while($categoryrs = $db->fetch_array($catequery)){
	    	$db->query("DELETE FROM sdw_image_cat WHERE cid=".$categoryrs['cid']);
	    	if ($islog){
...
if($_GET['action']=='edit'){
    ...
    $category = $db->get_one("SELECT * FROM sdw_image_cat WHERE cid=$cid");
    if(!empty($category)){
    	$smarty->assign('categories',get_image_category($category['cup']));
	...
	$smarty->assign('categories',get_image_category($cup));
}
$smarty->assign('manage_title',$LANG['image_cat_manage']);
...
function checkcategory($name){
	global $db;
	if (!empty($name)){	
	    $category = $db->get_one("SELECT name FROM sdw_image_cat WHERE name='$name'");
